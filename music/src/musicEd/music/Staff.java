...
public class Staff extends Mass {
    ...
    public Staff(Sys sys, int iStaff) {
        ...
        addReaction(new Reaction("S-S") { // create bar line
            public int bid(Gesture gest) {
                ...
                return d < 30 ? d + UC.marginSnap : UC.noBid; //we bias to prefer bar cycle barType
            }
            public void act(Gesture gest) {
                int rightMargin = Page.PAGE.xMargin.hi;
                int x = gest.vs.xM();
                if (x > rightMargin - UC.marginSnap) {
                    x = rightMargin;
                }
        ...
        addReaction(new Reaction("S-S") { // toggle barContinues
            public int bid(Gesture gest) {
                if(Staff.this.sys.iSys != 0) {return UC.noBid;} //only bar continues in first Sys
                int y1 = gest.vs.yL(), y2 = gest.vs.yH();
                int iStaff = Staff.this.iStaff;
                int iLastStaff = Page.PAGE.sysFmt.size() - 1;
                if (iStaff == iLastStaff ) { // last staff cannot continue
                    return UC.noBid;
                }
        ...
        addReaction(new Reaction("E-S") { // Create one eighth rest
            public int bid(Gesture gest) {
                ...
                G.LoHi m = Page.PAGE.xMargin;
                if (x < m.lo || y > m.hi) {return UC.noBid;}
                int H = Staff.this.H(), top = Staff.this.yTop(), bot = Staff.this.yBot();
                if (y < top || y > bot) {return UC.noBid;}
                return 10;
            }
            public void act(Gesture gest) {
    ...
    public static class Fmt{
        public int nLines = 5, H = 8; // H half line spacing
